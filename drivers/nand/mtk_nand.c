...
bool get_device_info(u16 id, u32 ext_id, flashdev_info * pdevinfo)
{
    ...
    for (index = 0; gen_FlashTable[index].id != 0; index++)
    {
        if (id == gen_FlashTable[index].id && ext_id == gen_FlashTable[index].ext_id)
        {
            ...
            printk(KERN_INFO "Device found in MTK table: %s (oobsize=%d)\n", pdevinfo->devicename, pdevinfo->sparesize);
...
static int mtk_nand_verify_buf(struct mtd_info *mtd, const uint8_t * buf, int len)
{
    ...
    for (i = 0; i < len; ++i)
    {
        if (*pSrc != *pDst)
        {
            printf("mtk_nand_verify_buf page fail at page %d, byte 0x%X (%X!=%X)\n", pkCMD->u4RowAddr, i, *pSrc, *pDst);
            return -1;
        }
        pSrc++;
        pDst++;
    ...
    {
        printf( "mtk_nand_verify_buf oob fail at page %d\n", pkCMD->u4RowAddr);
...
int read_fact_bbt(unsigned int page)
{
	...
	printf("read_fact_bbt at page 0x%X\n",page); 
...
int write_fact_bbt(u32 page)
{
	...
	printf("write_fact_bbt at page 0x%X\n",page); 
...
int load_fact_bbt()
{
	...
	printf("No fact_bbt. Creating it...\n");
...
//        MSG(INIT, "[mtk_nand] probe successfully!\n");
        printf( "writesize=%d, oobsize=%d, erasesize=%d, iowidth=%d\n",mtd->writesize,mtd->oobsize, mtd->erasesize,devinfo.iowidth);
        nand_disable_clock();
        return err;
    }
...
int ranand_write(char *buf, unsigned int to, int datalen)
{
	...
	while (datalen > 0) {
		...
		if (ret) {
			...
			printf("mtk_nand_write_page error %d\n", ret);
			return -1;
		}
#ifdef CONFIG_BADBLOCK_CHECK
		mtk_nand_command_bp(mtd, NAND_CMD_READ0, 0, page);
...
int ranand_erase_write(char *buf, unsigned int offs, int count)
{
	...
	printf("ranand_erase_write: %p, 0x%x, 0x%x\n",buf, offs, count);
...
#define BLOCK_ALIGNED(a) ((a) & (blocksize - 1))
		...
		if (BLOCK_ALIGNED(offs) || (count < blocksize))
		{
			...
			printf("%s: malloc block block_orig=0x%x, block=0x%x, blockaddr=0x%x, blocksize=0x%x, blockmask=0x%x \n", __func__,block_orig,block,blockaddr,blocksize,blockmask);
try_next_0:
			...
			//printf("offs=0x%x piece=0x%x piece_size=0x%x\n",offs,piece,piece_size);
			rc = ranand_erase(blockaddr, blocksize);
			//printf("erase rc=%d, blockaddr=0x%x, blocksize=0x%x\n",rc,blockaddr,blocksize);
//			ra_dbg("(%d)offs=%d piece=%d piece_size=%d rc=%d\n",__LINE__,offs,piece,piece_size,rc);
			...
			printf("write block block=0x%x, blockaddr=0x%x, blocksize=0x%x \n",block,blockaddr,blocksize);
			rc = ranand_write(block, blockaddr, blocksize);
...
try_next_1:
	            //printf("ranand_erase_write: erase 0x%x, 0x%x\n",offs, aligned_size);
                line_count++;
	            if (line_count > 39){
	                line_count = 0;
                    printf("\n\t");
			...
			rc = ranand_erase(offs, aligned_size);
	            //printf("ranand_erase_write: erase res:%d\n",rc);
#ifdef CONFIG_BADBLOCK_CHECK
			if (rc >= 1) {
			...
			if (rc != 0)
			{
		        ...
		        //printf("ranand_erase_write: write %p, 0x%x, 0x%x\n", buf, offs, aligned_size);
			rc = ranand_write(buf, offs, aligned_size);
			    //printf("ranand_erase_write: write res:%d\n",rc);
			if ( rc != aligned_size)
			{
				return -1;
			}
...
int ralink_nand_command(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if (!strncmp(argv[1], "id", 3)) {
        ...
        else if (!strncmp(argv[1], "write.uboot", 12)) {
                printf("Write 0x40000 bytes from memory (0x80100000) to NAND (0x00000000)\n");
                ranand_erase_write((char *)0x80100000 , 0, 0x40000);
        }
        else if (!strncmp(argv[1], "write.mem", 10)) {
		addr = (unsigned int)simple_strtoul(argv[2], NULL, 16); //addr
		page_addr = (unsigned int)simple_strtoul(argv[3], NULL, 16); //page
		len = (int)simple_strtoul(argv[4], NULL, 16);

                printf("Write from memory to NAND\n");

                //this function depend on spi/nand build env? CHECK IT!!!
                ranand_erase_write((char *)addr , page_addr, len);
        }
...
U_BOOT_CMD(
	...
	"  nand dump\n"
	"  nand init\n"
	"  nand woob <page> <offset> <data...>\n"
	"  nand rawe <addr> <len>\n"
	"  nand markb <addr>\n"
	"  nand write.mem <mem_addr> <nand_addr> <len>\n"
	"  nand write.uboot\n"
	"            write 0x40000 bytes from 0x80100000\n"
);
