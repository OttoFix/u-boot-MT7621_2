...
//						printf("\n\nAll data received (%d of %d). Fail_flag=%d\n", hs->upload, hs->upload_total, webfailsafe_upload_failed);
						printf("\n\n");
						...
						if(!webfailsafe_upload_failed){
							fs_open(file_flashing_html[0].name, &fsfile);
						} else {
							fs_open(file_fail_html[0].name, &fsfile);
						}
						...
						hs->dataptr = (u8_t *)fsfile.data;
						hs->upload = fsfile.len;
//						printf("uip_send (upload_len=%d, uip_mss=%d)\n", hs->upload, uip_mss());
						uip_send(hs->dataptr, (hs->upload > uip_mss() ? uip_mss() : hs->upload));
//						printf("sent done\n");
					}
