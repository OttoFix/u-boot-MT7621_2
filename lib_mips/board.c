...
static int display_banner(void)
{
    ...
    printf ("\n\n\n");
    printf ("                             _____                 \n"); 
    printf ("  ____                     _' ___ '  ___    _____  \n");
    printf (" |  _ \\ ___ _ __ ___ _ __ (_)'___'  |_ _|__|_   _| \n");
    printf (" | |_) / _ \\ '__/ _ \\ '_ \\| |/ _ \\   | |/ _ \\| |   \n");
    printf (" |  __/  __/ | |  __/ | | | | (_) |  | | (_) | |   \n");
    printf (" |_|   \\___|_|  \\___|_| |_|_|\\___/  |___\\___/|_|   \n");
...
int flash_uboot_image(ulong image_ptr, ulong image_size)
{
...
#if defined (CFG_ENV_IS_IN_NAND)
	printf("\n Copy %d bytes to Flash NAND... \n", image_size);
	rrc = ranand_erase_write((char *)image_ptr, 0, image_size);
#elif defined (CFG_ENV_IS_IN_SPI)
	printf("\n Copy %d bytes to Flash SPI... \n", image_size);
	...
	printf("U-Boot image flash Ok \t\t%d bytes flashed\n", image_size);
	return 0;
}

int flash_kernel_image(ulong image_ptr, ulong image_size)
{
	uint8_t factory = 1;  // enable flash factory
	int rrc = 0;
	ulong e_end;
...
/*	if (verify_kernel_image(image_ptr, NULL, NULL, NULL) <= 0) {
...
#if defined (CFG_ENV_IS_IN_NAND)
	printf("\nStart flashing FACTORY image to NAND\n");
	printf("Copy %d bytes to Flash... \n", image_size);
	//rrc = ranand_erase_write((char *)image_ptr, CFG_FACTORY_ADDR-CFG_FLASH_BASE, image_size);
	//rrc = ranand_erase_write((char *)image_ptr, CFG_KERN_ADDR-CFG_FLASH_BASE, image_size);
	rrc = ranand_erase_write((char *)image_ptr, 0x600000, image_size);   /*** correct addresses factory FS ??? fix later  ***/
	printf("Copy done\n ");
#elif defined (CFG_ENV_IS_IN_SPI)
	printf("\n Copy %d bytes to Flash... \n", image_size);
	...
	if (factory==true){
		e_end = CFG_FACTORY_ADDR + image_size - 1;
		if (get_addr_boundary(&e_end) == 0) {
			printf("\n Erase %s block from 0x%X to 0x%X\n", "Linux", CFG_FACTORY_ADDR, e_end);
			flash_sect_erase(CFG_FACTORY_ADDR, e_end);
			printf("\n Copy %d bytes to Flash... \n", image_size);
			rrc = flash_write((uchar *) image_ptr, CFG_FACTORY_ADDR, image_size);
		} else {
			rrc = ERR_ALIGN;
		}
	...
	printf("Kernel image flash Ok \t\t%d bytes flashed\n", image_size);
...
int dummy_flash_erase_write(char *buf, unsigned int offs, int count)
{
    printf("\ndummy_flash_erase_write: Your flash is not supported!!! Skipping...\n\n");
    return 0;
}
...
#if CFG_CMD_HTTPD
       // current_flash_erase_write = dummy_flash_erase_write;
#endif
...
#if CFG_CMD_HTTPD
        //current_flash_erase_write = ranand_erase_write;
#endif
	bd->bi_flashstart = 0;
...
*/
	...
	bd->bi_flashoffset = 0;
#if CFG_CMD_HTTPD
	//current_flash_erase_write = raspi_erase_write;
#endif
#else //CFG_ENV_IS_IN_FLASH
...
#if defined(MT7621_DDR_SPEED) 
		switch(MT7621_DDR_SPEED) {
		    case 0x11:
			    ddr_speed = 1200;
			    break;
		    case 0x21:
...
/**** update firmware if present on USB *****/
	...
	if (!flash_kernel_image_from_usb(cmdtp)){
		...
		ra_outl(0xbe000620, (ra_inl(0xbe000620) & ~(1U << 8)));//GPIO  6 off
		ra_outl(0xbe000620, (ra_inl(0xbe000620) & ~(1U << 6))); // GPIO 6 on
		//ra_outl(0xbe000620, (ra_inl(0xbe000620) | (1U << 23)));//led 6 off  ////23
		printf("gpio value reg: %x \n",(ra_inl(0x1e000620)) );
		printf("gpio direction reg: %x \n",(ra_inl(0x1e000600)) );
		printf("gpio mode reg: %x \n",(ra_inl(0x1e000060)) );
		while (1){
